...
# Code generation (overwrites src/store/api.gen.ts — never edit by hand)
pnpm api:gen             # @rtk-query/codegen-openapi; spec pulled from the service repo (see src/store/openapi-config.cjs)
```
...
backend spec and fails if `src/store/api.gen.ts` differs — so the generated client is committed and must stay in
...
- **`baseApi.ts`** — hand-written `createApi` base: `fetchBaseQuery` with a `prepareHeaders` callback that attaches the
  ...
  `openapi-config.cjs`) and must never be overwritten.
- **`api.gen.ts`** — generated by `pnpm api:gen`. One endpoint per OpenAPI operation, injected onto `baseApi` via
...
Endpoints and request/response types are generated into `src/store/api.gen.ts` by `pnpm api:gen`
(`@rtk-query/codegen-openapi`, spec pulled from the service repo — see `src/store/openapi-config.cjs`). **Never edit
`api.gen.ts` by hand** — it's overwritten on regeneration. Import types from `api.gen.ts`; consume hooks from
`src/store/api.ts`.
